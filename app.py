...
st.selectbox(label= 'Choose Plate Mode', options= ['Cretae New Plate', 'Use Ready Plate'])
#st.set_page_config(page_title="VisionCraft: Empowering Experts to Create Custom Color Blindness Tests", layout="centered")
...
with col2:
 st.image("ishihara.png", width=250) 

# --- User inputs ---
number = st.text_input("Enter the number to display:", "12")
size = st.slider("Image size (pixels):", 300, 1000, 500, 50)
num_dots = st.slider("Number of dots:", 500, 5000, 2500, 100)
min_r = st.slider("Minimum dot radius:", 2, 10, 4)
max_r = st.slider("Maximum dot radius:", 4, 20, 8)

if st.button("Generate Plate"):
    with st.spinner("Generating plate..."):
        ...
        st.download_button("Download Plate", data=buf, file_name=f"ishihara_{number}.png", mime="image/png")
...
with right_col:
    if mode == "Create New Plate":
        ...
        num_number_colors = st.slider("How many Number Colors to use?", 1, 6, 3)
        number_colors = []
