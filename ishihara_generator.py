...
def generate_ishihara_with_number(size=500, number='69', radius_range=(4, 8), num_dots=2500, spacing_factor=1):
    ...
    while len(placed_dots) < num_dots and attempts < max_attempts:
        ...
        too_close = any((x - px) ** 2 + (y - py) ** 2 < (spacing_factor * (r + pr)) ** 2 for (px, py, pr) in placed_dots)
        if too_close:
            attempts += 1
            continue
